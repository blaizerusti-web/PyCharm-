...
class AIBackend:
    ...
    def _openai_chat(self, messages: List[Dict[str, Any]], model: str | None,
                     max_tokens: int = 800, timeout: float = 60.0) -> str:
        if not OPENAI_KEYS:
            ...
            try:
                if self._new_sdk_cls:
                    ...
                    r = cli.chat.completions.create(
                        model=model or self.openai_model, messages=messages, max_tokens=max_tokens
                    )
                    return r.choices[0].message.content.strip()
                elif self._old_sdk:
                    self._old_sdk.api_key = key
                    r = self._old_sdk.ChatCompletion.create(
                        model=model or self.openai_model, messages=messages,
                        max_tokens=max_tokens, request_timeout=timeout
                    )
                    return r["choices"][0]["message"]["content"].strip()
                else:
                ...
                last_error = e
                msg = str(e).lower()
                retriable = any(t in msg for t in
                                ["rate limit", "quota", "429", "timeout", "temporarily", "connection", "service unavailable"])
...
cur.execute("""CREATE TABLE IF NOT EXISTS raw_events(
  ...
  type TEXT NOT NULL, text TEXT NOT NULL, meta TEXT)""")
cur.execute("CREATE INDEX IF NOT EXISTS idx_raw_ts ON raw_events(ts)")
cur.execute("""CREATE TABLE IF NOT EXISTS notes(
  id INTEGER PRIMARY KEY AUTOINCREMENT, ts TEXT NOT NULL,
  source TEXT NOT NULL, topic_key TEXT NOT NULL,
...
def log_raw(ev_type: str, text: str, meta: dict | None = None) -> int:
    ...
    cur.execute("INSERT INTO raw_events(ts,type,text,meta) VALUES(?,?,?,?)",
                (ts, ev_type, text, m))
    conn.commit()
    rid = cur.lastrowid
    _append_jsonl({"id": rid, "ts": ts, "type": ev_type,
                   "text": text, "meta": meta or {}})
    logging.info("raw[%s] %s: %s", rid, ev_type,
...
def _merge_contents(old: str, new: str) -> str:
    try:
        return AI.chat(
            [{"role": "system",
              "content": "Merge NEW into EXISTING. ≤120 words, bullets ok. Preserve key facts/numbers."},
...
def remember(source: str, text: str, raw_ref: str = "") -> int:
    ...
    if row:
        ...
        cur.execute("""UPDATE notes
                       ...
                       WHERE id=?""",
                    (datetime.utcnow().isoformat(), source, digest["title"],
                     merged, digest["tags"], digest["sentiment"], raw_ref, nid))
        conn.commit()
        return nid
    cur.execute("""INSERT INTO notes(ts,source,topic_key,title,content,tags,sentiment,raw_ref)
                   VALUES(?,?,?,?,?,?,?,?)""",
                (datetime.utcnow().isoformat(), source, topic,
                 digest["title"], digest["summary"], digest["tags"],
                 digest["sentiment"], raw_ref))
    conn.commit()
    ...
    for r in cur.fetchall():
        ...
        if score > 0:
            raw_scored.append((score, {
                "id": r[0], "ts": r[1], "type": r[2], "text": txt, "meta": r[4]
            }))
...
def analyze_image(path: Path) -> str:
    ...
    try:
        ...
        gist = (
            f"Image {path.name}: EXIF {exif if exif else '∅'}; "
            f"OCR preview: {(ocr[:500] + '…') if ocr and len(ocr) > 500 else (ocr or '∅')}"
        )
            ...
            if rec:
                ...
                if row:
                    cur.execute("UPDATE notes SET ts=?,source=?,title=?,content=?,tags=?,sentiment=? WHERE id=?",
                                (datetime.utcnow().isoformat(), "system", "Persona", persona,
                                 "persona,profile", "", row[0]))
                else:
                    cur.execute("""INSERT INTO notes(ts,source,topic_key,title,content,tags,sentiment,raw_ref)
                                   VALUES(?,?,?,?,?,?,?,?)""",
                                (datetime.utcnow().isoformat(), "system", tk, "Persona", persona,
                                 "persona,profile", "", ""))
                conn.commit()
        except Exception as e:
...
def tts_to_mp3(text: str) -> bytes:
    ...
    try:
        ...
        key = OPENAI_KEYS[0]
        cli = _New(api_key=key)
        r = cli.audio.speech.create(model="gpt-4o-mini-tts", voice=OPENAI_TTS_VOICE,
